...
int request_call(struct shared_struct * shared){
	...
	while(1){
		...
		if(i == 1){ // Add mode
			...
			cell.flag = ALIVED;
			printf("[add] Input the ID of student: ");
			scanf("%d",&(cell.id));
			printf("[add] Input the name of student: ");
			scanf("%s",cell.name);
			...
			if(j != shared->size + 1){
				printf("[delete] That id has already existed\n");
			}
			else{
				push(cell, shared);
			}
			...
			int id,j;
			printf("[delete] Input the ID of student: ");
			scanf("%d",&(id));
			for(j = 1; j < shared->size + 1; j++){
			...
			if(j == shared->size + 1){
				printf("[delete] No that id\n");
			}
		}
		else if(i == 3){ // Modify Mode
			int id, j;
			printf("[modify] Input the ID of student: ");
			scanf("%d",&(id));
			for(j = 1; j < shared->size + 1; j++){
			...
			if(j == shared->size + 1){
				printf("[modify] No that id\n");
			}
			else{
				printf("[modify] Original: %8d   %s\n",shared->heap[j].id, shared->heap[j].name);
				printf("[modify] Input the new name of student: ");
				scanf("%s",shared->heap[j].name);
			}			
		}
		else if(i == 4){ // Find Mode
			int id, j;
			printf("[find] Input the ID of student: ");
			scanf("%d",&(id));
			for(j = 1; j < shared->size + 1; j++){
				if(shared->heap[j].id == id && shared->heap[j].flag == ALIVED){
					break;
			...
			if(shared->size == 0){
				printf("--------------------------\n");
				printf("[show] Empty Data table\n");
				printf("--------------------------\n");
				break;
			}
			printf("--------------------------\n");
			printf("[show] Heap size: %d\n", shared->size);
			printf("--------------------------\n");
			printf("[show] Data table\n");
			printf(" Num      ID      Name\n");
			...
			printf("[inner] Heap size: %d\n", shared->size);
			printf("[inner] Inner data array\n");
			for(int i = 0; i < 11; i++){
				printf("|%3d|%3d|%8d|%10s|\n",i,shared->heap[i].flag,shared->heap[i].id,shared->heap[i].name);
			}
		}
