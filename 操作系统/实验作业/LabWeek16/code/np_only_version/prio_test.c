...
static void *runner(void *par){
	...
	printf("Thread %d works %d second(s), with priority %d\n",cn->id, cn->worst_time, cn->priority);
	sleep(cn->worst_time);
	scheduler_show(&sche);
...
int main(){
	...
	printf("------------PRIO TEST INPUT------------\n");
	printf("enter the args of prio-test\n");
	printf("NOTICE: start time must be incremental\n");	
	printf("thread num: ");
	scanf("%d",&pnum);
	if(pnum <= 0){
		printf("prio_test(): invalid thread num\n");
		return 1;
	}
	int run_t[pnum],start_t[pnum],pri[pnum];
	for(int i = 0; i < pnum; i++){
		printf("start time (sec) of thread-%d: ", i+1);
		scanf("%d",start_t+i);
		if(start_t[i] < 0){
			printf("prio_test(): invalid start time\n");
			return 1;
		}
	}
	for(int i = 0; i < pnum; i++){
		...
		scanf("%d",run_t+i);	
		if(run_t[i] < 0){
			printf("prio_test(): invalid run time\n");
			return 1;
		}
	}
	for(int i = 0; i < pnum; i++){
